...
from socks_proxy_win.updater import (
    UpdateInfo,
    check_for_update,
    download_update,
)
...
class TestCheckForUpdate:
    ...
    async def test_newer_version_available(self, mock_session):
        ...
        response.json = AsyncMock(return_value={
            "tag_name": "socks-exe-v2.0.0",
            "assets": [{"name": "netbridge-socks.exe", "browser_download_url": "https://example.com/netbridge-socks.exe"}],
        })
        with patch("socks_proxy_win.updater.APP_VERSION", "1.0.0"):
            result = await check_for_update(session)
        assert result is not None
        assert result.version == "2.0.0"
        assert result.download_url == "https://example.com/netbridge-socks.exe"

    async def test_same_version_returns_none(self, mock_session):
        session, response = mock_session
        response.json = AsyncMock(return_value={
            "tag_name": "socks-exe-v1.0.0",
            "assets": [{"name": "netbridge-socks.exe", "browser_download_url": "https://example.com/netbridge-socks.exe"}],
        })
        with patch("socks_proxy_win.updater.APP_VERSION", "1.0.0"):
            result = await check_for_update(session)
    ...
    async def test_dev_version_skips_check(self, mock_session):
        ...
        with patch("socks_proxy_win.updater.APP_VERSION", "0.0.0"):
            result = await check_for_update(session)
        assert result is None
    ...
    async def test_non_socks_exe_tag_skipped(self, mock_session):
        """Tags like agent-v2.0.0 should not trigger socks-exe updates."""
        session, response = mock_session
        response.json = AsyncMock(return_value={
...
class TestDownloadUpdate:
    async def test_downloads_to_dest(self, tmp_path):
        dest = tmp_path / "netbridge-socks.exe"
        ...
        result = await download_update(session, "https://example.com/netbridge-socks.exe", dest)
        assert result == dest
    ...
    async def test_download_failure_raises(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="Download failed"):
            await download_update(session, "https://example.com/netbridge-socks.exe", dest)
